...
def serve_focus(paire_id: str) -> None:
    ...
    ############## 1ST BAND - SERVE EFFICIENCY PER GAME ##############
    ...
    for game_id in game_ids:
        ...
        undetermined_count = results_df[
            (results_df["game_id"] == game_id) & (results_df["grade"] == "undetermined")
        ].shape[0]
        error_count = results_df[
            (results_df["game_id"] == game_id) & (results_df["grade"] == "serve error")
        ].shape[0]
        excellent_pass_count = results_df[
            (results_df["game_id"] == game_id)
            & (results_df["grade"] == "excellent pass (++)")
        ].shape[0]
        good_pass_count = results_df[
            (results_df["game_id"] == game_id)
            & (results_df["grade"] == "good pass (+)")
        ].shape[0]
        average_pass_count = results_df[
            (results_df["game_id"] == game_id)
            & (results_df["grade"] == "average pass (0)")
        ].shape[0]
        bad_pass_count = results_df[
            (results_df["game_id"] == game_id) & (results_df["grade"] == "bad pass (-)")
        ].shape[0]
        ace_count = results_df[
            (results_df["game_id"] == game_id) & (results_df["grade"] == "ace")
        ].shape[0]
        ...
        serve_grade_counts_df = pd.DataFrame.from_dict(
            serve_grade_counts, orient="index"
        )

        undetermined_ratio = (
            undetermined_count / total_serves if total_serves > 0 else 0
        )
        error_ratio = error_count / total_serves if total_serves > 0 else 0
        excellent_pass_ratio = (
            excellent_pass_count / total_serves if total_serves > 0 else 0
        )
        good_pass_ratio = good_pass_count / total_serves if total_serves > 0 else 0
        average_pass_ratio = (
            average_pass_count / total_serves if total_serves > 0 else 0
        )
        ...
        paire_name = (
            results_df[results_df["game_id"] == game_id]["paire_name"]
            .iloc[0]
            .replace(" - ", "<br>")
        )
    ...
    serve_grade_counts_df = pd.DataFrame.from_dict(
        filtered_serve_grade_counts, orient="index"
    )
    serve_grade_ratios_df = pd.DataFrame.from_dict(
        filtered_serve_grade_ratios, orient="index"
    )
    ...
    fig_1 = go.Figure(
        data=[
            go.Bar(
                name="Indéterminé",
                x=serve_grade_ratios_df.index,
                y=serve_grade_counts_df["undetermined_count"],
                marker_color=grade_color_dict["undetermined"],
            ),
            go.Bar(
                name="Faute service",
                x=serve_grade_ratios_df.index,
                y=serve_grade_counts_df["error_count"],
                marker_color=grade_color_dict["serve error"],
            ),
            go.Bar(
                name="Réception parfaite (++)",
                x=serve_grade_ratios_df.index,
                y=serve_grade_counts_df["excellent_pass_count"],
                marker_color=grade_color_dict["excellent pass (++)"],
            ),
            go.Bar(
                name="Bonne réception (+)",
                x=serve_grade_ratios_df.index,
                y=serve_grade_counts_df["good_pass_count"],
                marker_color=grade_color_dict["good pass (+)"],
            ),
            go.Bar(
                name="Réception moyenne (0)",
                x=serve_grade_ratios_df.index,
                y=serve_grade_counts_df["average_pass_count"],
                marker_color=grade_color_dict["average pass (0)"],
            ),
            go.Bar(
                name="Mauvaise réception (-)",
                x=serve_grade_ratios_df.index,
                y=serve_grade_counts_df["bad_pass_count"],
                marker_color=grade_color_dict["bad pass (-)"],
            ),
            go.Bar(
                name="Ace",
                x=serve_grade_ratios_df.index,
                y=serve_grade_counts_df["ace_count"],
                marker_color=grade_color_dict["ace"],
            ),
        ]
    )
    fig_1.update_layout(
        barmode="stack",
        title="Total des services par match",
        xaxis=dict(
            tickvals=serve_grade_ratios_df.index.tolist(),
            ticktext=serve_grade_ratios_df["paire_name"].tolist(),
        ),
    )
    col_barplot1.plotly_chart(fig_1, use_container_width=True)
    ...
    fig_2 = go.Figure(
        data=[
            go.Bar(
                name="Indéterminé",
                x=serve_grade_ratios_df.index,
                y=serve_grade_ratios_df["undetermined_ratio"],
                marker_color=grade_color_dict["undetermined"],
            ),
            go.Bar(
                name="Faute service",
                x=serve_grade_ratios_df.index,
                y=serve_grade_ratios_df["error_ratio"],
                marker_color=grade_color_dict["serve error"],
            ),
            go.Bar(
                name="Réception parfaite (++)",
                x=serve_grade_ratios_df.index,
                y=serve_grade_ratios_df["excellent_pass_ratio"],
                marker_color=grade_color_dict["excellent pass (++)"],
            ),
            go.Bar(
                name="Bonne réception (+)",
                x=serve_grade_ratios_df.index,
                y=serve_grade_ratios_df["good_pass_ratio"],
                marker_color=grade_color_dict["good pass (+)"],
            ),
            go.Bar(
                name="Réception moyenne (0)",
                x=serve_grade_ratios_df.index,
                y=serve_grade_ratios_df["average_ratio"],
                marker_color=grade_color_dict["average pass (0)"],
            ),
            go.Bar(
                name="Mauvaise réception (-)",
                x=serve_grade_ratios_df.index,
                y=serve_grade_ratios_df["bad_pass_ratio"],
                marker_color=grade_color_dict["bad pass (-)"],
            ),
            go.Bar(
                name="Ace",
                x=serve_grade_ratios_df.index,
                y=serve_grade_ratios_df["ace_ratio"],
                marker_color=grade_color_dict["ace"],
            ),
        ]
    )
    fig_2.update_layout(
        barmode="stack",
        title="Ratio des services par match",
        showlegend=False,
        xaxis=dict(
            tickvals=serve_grade_ratios_df.index.tolist(),
            ticktext=serve_grade_ratios_df["paire_name"].tolist(),
    ...
    col_barplot2.plotly_chart(fig_2, use_container_width=True)

    ############## 2ND BAND - SERVE EFFICIENCY AGAINST OPPONENTS ##############

    # Columns set up
    col_selectbox, col_widget_1, col_widget_2, col_widget_3 = st.columns(
        [1, 0.5, 0.5, 0.5]
    )
    col_barplot3, col_barplot4 = st.columns([1.4, 1])

    # Fetching the opponent team names for the selected team
    with col_selectbox:
        ...
        focus_game_ids = results_df[
            results_df["paire_name"] == selected_opponent_paire_name
        ]["game_id"].unique()
        focus_results_df = results_df[results_df["game_id"].isin(focus_game_ids)]

    # with col_barplot3:

    ############### [DEV] Displaying the data ##############
    full_width_container = st.container()
    full_width_container.dataframe(focus_results_df)
    # full_width_container.dataframe(results_df)
