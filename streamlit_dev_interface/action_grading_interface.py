...
def action_grading_interface():
    ...
    st.info("Paire name : {paire_name} - Paire ID : {paire_id}".format(
    ...
    selected_action = str(st.selectbox(label="Select an action to grade:",
                                       options=["serve","pass"]))

    # Service or passing grading interface
    if selected_action in ["serve", "pass"]:
        # Specific widgets for serve or pass grading
        col_toggle,col_recall,col_game_or_serie,col_launch_grading = st.columns([1, 1, 2, 1])
        with col_toggle:
            st.toggle(
                ...
                on_change=lambda: st.session_state.update({"selected_game_id": None, "selected_serie_id": None})
                )
