...
class FirestoreManager:
    ...
    def get_serve_data_df(
        self, paire_id: str, player: str | None = None
    ) -> pd.DataFrame:
        ...
        games_docs = (
            self.db.collection("games").where("team_a", "==", paire_id).stream()
        )
        ...
        serie_ids = list(
            {g.get("serie") for g in games_dict.values() if g.get("serie")}
        )
        ...
        team_b_ids = list(
            {g.get("team_b") for g in games_dict.values() if g.get("team_b")}
        )
        ...
        for i in range(0, len(game_ids), 30):
            ...
            for doc in (
                self.db.collection("points").where("game_id", "in", chunk).stream()
            ):
